...
contract TinyTalesDAO {
    ...
    function isStorageProvider(uint256 proposalID) view internal returns(bool) {
    ...
    function upvoteProposal(uint256 proposalID) public {
        require(!isStorageProvider(proposalID), "Storage Provider cannot vote his own proposal");
    ...
    function validatePolicy(uint256 proposalID) internal view returns (bool) {
    ...
    function authorizeData(uint256 proposalID, bytes calldata cidraw, bytes calldata provider, uint size) public {
        ...
        require(validatePolicy(proposalID), "Deal failed policy check: Was the CID proposal Passed?");
